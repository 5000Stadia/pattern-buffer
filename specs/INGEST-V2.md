...
## 8. Accepted next-cycle design (letter 019 rulings; 036 live evidence; building now)

- **Registry self-check — propose, never auto-merge.** A dedupe pass over
  the assembled registry emits MAYBE_SAME_AS edges carrying their
  evidence, generated by TWO independent signals — name/alias lexical
  overlap AND anchor overlap (roles, recurring locations, features) — per
  the Graphiti deep-dive's non-redundant-signals finding and whitepaper
  §11's brittle-single-anchor rule. **Signal weighting (036 live
  evidence):** name anchors outweigh role/title tokens — a crowd noun
  sharing a title register with a named principal ("officials" /
  "Allocation Officer Marn") must NOT clear the proposal threshold on
  title overlap alone; the live bad merge is the regression case;
- **Gate `same_as` demoted to proposal (036; the 019 principle applied
  to its last exception).** `ingest_structured`'s `same_as` item field
  currently performs an immediate `registry.merge` — extractor-granted
  merge authority from single-call context, exactly what 019 forbade for
  M|a|b. Change: the gate records `maybe_same_as` (with the extractor's
  evidence) instead of merging; promotion to a merge EVENT happens at
  pass-2/self-check/explicit confirmation where the whole world is in
  view. Engine-level change, additive in spirit (the field still exists;
  its effect becomes a proposal). Eval impact: the chapter test's
  late-binding criterion grades the merged OUTCOME — promotion at audit
  must therefore run in the pipelines (a deterministic promotion rule:
  a maybe_same_as whose two sides share a NAME-class anchor promotes;
  title-only candidates wait for tier-2/confirmation); merges remain logged events
