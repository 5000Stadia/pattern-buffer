...
**Set-valued attributes (run-4 finding):** `name`, `alias`, `connects_to`, `adjacent_to`, `same_as`, `maybe_same_as` are set-valued — multiple coexisting values are data, never contradiction; conflict detection applies to functional keys only (62 of run 4's 77 raw flags were multiplicity misread as dispute).

**Source classes, exactly (run-4 finding):** for supersession purposes `stated` and `observed` without document chains are ONE class (`direct`) — both rank-3 authoritative, and ordinary narrative movement supersedes across them; the §7.1 boundary that creates a second class is the document trust chain (`document`), and the corroborate-vs-flag machinery applies between `direct` and `document` (and between distinct documents).
