...
**Set-valued attributes (run-4 finding):** `name`, `alias`, `connects_to`, `adjacent_to`, `same_as`, `maybe_same_as` are set-valued — multiple coexisting values are data, never contradiction; conflict detection applies to functional keys only (62 of run 4's 77 raw flags were multiplicity misread as dispute). **Set-valuedness is only ever DECLARED** — this frozenset, or future registry metadata for domain attributes — **never inferred from observed multiplicity** (letter 019): else a real single-valued contradiction emitted twice reads as set growth.

**Source classes, exactly (run-4 finding):** for supersession purposes `stated` and `observed` without document chains are ONE class (`direct`) — both rank-3 authoritative, and ordinary narrative movement supersedes across them; the boundary's NAME is **direct-vs-document** (letter 019; supersedes any stated-vs-observed phrasing): the second class is the document trust chain (`document`), and the corroborate-vs-flag machinery applies between `direct` and `document` (and between distinct documents).

**Simultaneity guard (run-4 finding):** within-class supersession requires world-time *progression* — rows tied at the winning `valid_from` with differing values are a simultaneous contradiction: flagged, earliest-asserted served, never silently ordered by log sequence (log order is transaction bookkeeping, not a truth criterion). Genuine updates carry later valid_time; the extraction contract's fractional within-scene stamping is the cooperating discipline.
