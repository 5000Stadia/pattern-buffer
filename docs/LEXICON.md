...
| **`confidence()`** | A derived trust score for one functional folded key: `provenance × recency × corroboration` (halved if conflicted), plus `last_observed_at` for host-side staleness. Validity is untouched; never stored (the membrane). | Plain; the temporal-trust sibling of `salience` (relevance). |
